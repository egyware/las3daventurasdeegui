...
async function main(){    
    const options = stdio.getopt({
        ...
        'url': {key: 'u', args: 'string', description: 'Url a verificar'}
    });
    ...
    let promesas =
     options.proveedores.map(function(proveedorId) {
        ...
        .then(function (currentValue){                        
            ...
            crawlerData.validLinks    = alwaysFalse;
            crawlerData.validProducts = alwaysTrue;
            crawlerData.invalidProducts = alwaysFalse;
            crawlerData.origenes = [ options.url ]
            return crawlerData;
        })
        .then(crawler)        
    })   
